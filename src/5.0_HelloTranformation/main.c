...
run(void)
{
	while(!glfwWindowShouldClose(win)) {
		...
		glm_translate(trans, (vec3){radius * cosf(time_f), radius * sinf(time_f), 0.f});
		glm_rotate(trans, time_f, (vec3){0.f, 0.f, 1.f});
		...
		glUniformMatrix4fv(glGetUniformLocation(prog, "transform"), 1, GL_FALSE, trans[0]);
